...
create index if not exists user_cards_id_card on user_cards(id_card);

comment on column user_cards.name is '姓名';
comment on column user_cards.id_card is '身份证号码';
...
comment on column user_cards.id_card_addr is '身份证地址';
